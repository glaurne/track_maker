...
class TrackCreator:
    ...
    def __init__(self, root):
        ...
        #meters to pixels coversion
        #show pixels x and y axis - change to meters later on
        self.canvas = tk.Canvas(root, width= 800, height= 600, bg="white") #CANVAS FOR TRACK
        self.canvas.pack()
        ...
        for i in range(0,40):
            self.canvas.create_line(0, height, 800, height, fill='light grey')
            self.canvas.create_text(20, height + 10, text= int(height/10), fill='black') # y-axis labeling

            height += 20

        for i in range(0,60):
            self.canvas.create_line(width, 0, width, 600, fill='light grey')
            self.canvas.create_text(width + 15, 10, text= int(width/10), fill='black') # x-axis labeling
            width += 20
    ...
    def create_widgets(self):
        ...
        color_options = ["orange", "yellow", "blue", "red"]
    ...
    def place_cone(self, event):
        ...
        if self.mode == "add":
            ...
            for item in items:
                if self.canvas.gettags(item)[0] in ["orange", "yellow", "blue", "red"]:
    ...
    def submit(self):
        ...
        if red_cone:
            red_x, red_y = red_cone
            print("Blue cones:")
            for cone_id, x, y, color in self.cones:
                x_meters = (x - red_x) / 10  # Assuming 10 pixels = 1 meter
                y_meters = (y - red_y) / 10
                if color == "blue":
                    print("- -", y_meters)
                    print("-", x_meters)
            ...
            print("\nOrange cones:")
            print("- - 0")  # Orange cones is around red cones set to (0,0)
            print("-", 0)
            orange_coord += "- - 0\n"
            orange_coord += "- 0\n"
